-- ============================================================================
-- SIFTED INSIGHT RANKING v3 WITH CLUSTERING
-- ============================================================================
...
BEGIN
    ...
    v_keywords := ARRAY[
        SELECT CASE WHEN v_combined LIKE ('%' || keyword || '%') THEN keyword END
        FROM unnest(ARRAY[
            'gpt', 'chatgpt', 'anthropic', 'llm', 'ai', 'machine learning',
            'iphone', 'ipad', 'apple', 'vision pro',
            'tesla', 'robotaxi', 'elon musk',
            'bitcoin', 'ethereum', 'crypto', 'blockchain',
            'climate', 'global warming', 'carbon',
            'election', 'trump', 'biden',
            'stock market', 'fed', 'inflation',
            'microsoft', 'copilot', 'azure',
            'google', 'gemini', 'android',
            'twitter', 'x', 'instagram', 'facebook', 'meta', 'tiktok'
        ]::TEXT[]) AS kw(keyword)
        WHERE v_combined LIKE ('%' || kw.keyword || '%')
    ];
    
    -- Hash the keywords for clustering
    v_hash := encode(digest(COALESCE(array_to_string(v_keywords, ','), ''), 'md5');
    
    -- Determine category
    v_category := CASE
        WHEN v_combined LIKE '%gpt%' OR v_combined LIKE '%ai%' OR v_combined LIKE '%llm%' THEN 'AI'
        WHEN v_combined LIKE '%iphone%' OR v_combined LIKE '%apple%' OR v_combined LIKE '%vision pro%' THEN 'Apple'
        ...
        WHEN v_combined LIKE '%climate%' OR v_combined LIKE '%global warming%' THEN 'Climate'
    ...
    VALUES (article_uuid, v_hash, v_keywords, array_to_string(v_keywords, ', '), v_category, 
            LEAST(array_length(v_keywords, 1)::DECIMAL / 3, 1.0), TRUE)
...
BEGIN
    ...
    -- Get engagement (0 if table doesn't exist)
    BEGIN
    ...
    -- Get utility (word compression ratio)
