...
DECLARE
    ...
    article_title TEXT;
BEGIN
    -- Get tags and title from source article
    SELECT tags, title INTO article_tags, article_title
    ...
    SELECT 
        ...
        (
            ...
            CASE WHEN a.tags && article_tags THEN 3 ELSE 0 END +
            -- Title word overlap
            CASE WHEN LOWER(a.title) LIKE '%' || LEFT(LOWER(article_title), 20) || '%' THEN 2 ELSE 0 END +
            -- Recency boost
            CASE WHEN a.published_at >= NOW() - INTERVAL '7 days' THEN 1 ELSE 0 END
        ) AS similarity_score
    FROM articles a
    WHERE a.status = 'published'
...
COMMENT ON FUNCTION get_related_articles IS 'Find similar articles based on tags and title';
